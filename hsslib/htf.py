...
class HTF:
    ...
    def hss_to_htf(self, tol=1e-10):
        ...
        return self.model.pss.toeC @ (
            np.linalg.inv(sI + self.model.pss.Nblk - self.model.pss.toeA)
            @ self.model.pss.toeB
        )
    ...
    def plot(self, Nplot=None, axs=np.empty(0), xscale="linear", yscale="linear"):
        ...
        if axs.size == 0:
            fig, axs = plt.subplots(
                Nplot * 2 + 1, Nplot * 2 + 1, sharex="col", sharey="row"
            )
    ...
    def get_mid_col(self, tol=1e-2, N=6):
        ...
        for i in range(self.N * 2 + 1):
            if (
                max(abs(self.htf[:, i, self.N]) / maxhtf > tol)
                and abs(i - self.N) < N + 1
            ):
    ...
    def siso_eq(self, other):
        ...
        Hsiso = np.reshape(H0, (len(self.freqs), 1, 1)) - np.matmul(
            np.matmul(a, fbk), np.matmul(otherQ, b)
        )
