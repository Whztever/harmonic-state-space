...
class ParametricSweep:
        ...
        for idx_i, ival in enumerate(pspace[0]):
            if len(pspace) > 1:
                for idx_j, jval in enumerate(pspace[1]):
                    ...
                    print(
                        f"\r ({self.model.p[0].name}:{ival:.3}) ({self.model.p[1].name}:{jval:.3}) Converged in iteration {i + 1}, err: {err:.2e}",
                        end="",
                    )
    ...
    def eigenloci(self, pspace, p_idx=0):
        ...
        for indi, ival in enumerate(pspace):
            ...
            i, err = self.model.find_pss()
            print(
                f"\r ({self.model.p[p_idx].name}:{ival}) Converged in iteration {i + 1}, err: {err:.2e}",
                end="",
            )
            self.model.calc_modal_props()
            for xi in range(len(modal_props.eig_x0)):
                pfs_i = (
                    modal_props.pf_x0[:, xi].tolist()
                    + modal_props.npf[:, xi].tolist()
                    + [modal_props.eig_x0[xi].real]
                    + [modal_props.eig_x0[xi].imag]
                    + [ival]
                )
    ...
    def save_eigenloci(self, params, params_txt):
        ...
        param_dict = {
            "name": param_name,
            "values": paramvals,
            "poly": Polygon([(0, 1.7), (0, 1.9), (2, 1.9), (2, 1.7)]),
        }
    ...
    def eigenloci_plot(self):
        ...
        cmap = plt.get_cmap("cividis")
        ax1.set_title(
            f"Parametric eigenvalue study of a {self.__class__.__name__}\n "
            f"{param_name} swept from {paramvals[0]:.2g} to {paramvals[-1]:.2f}",
            color="k",
        )
        ...
        for ind in indicators.keys():
            ...
            indicators[ind]["txt"] = ax2.text(
                txt_x, txt_y, n.ljust(len(n)), ha="left", fontsize=8
            )
            txt_y -= dy
        ...
        def hover(event):
            ...
            if event.inaxes == ax1:
                ...
                if cont:
                    ...
                    points = "{}".format(
                        ", ".join([f"{paramvals[n]:.4g}" for n in ind["ind"]])
                    )
            ...
            # check if event was in axis 2
                ...
                for ind in indicators.keys():
                    ...
                    if indicators[ind]["poly"].contains(
                        Point(event.xdata, event.ydata)
                    ):
                        # clean axis 2 and change color map
                        clean_ax2()
                        change_opacity(
                            indicators[ind]["values"], indicators[ind]["txt"]
                        )
                        reset_flag = False
                        break
    ...
    def weakest_damping_3d(self, ax=None, offset=0):
        ...
        ax.plot_wireframe(
            X, Y, Z, rstride=2, cstride=2, alpha=0.25, color="k"
        )  # cmap=cm.Greys)
