...
class HSS:
    ...
    def find_pss(self):
        ...
        for i in range(20):
            self.calc_var_td()
            self.pss.ft = self.calc_lam_td(
                self.f_lam, self.pss.vars_t
            )  # Calc f in time domain
            self.pss.At = self.calc_lam_td(
                self.A_lam, self.pss.vars_t
            )  # Calc A in time domain
            ...
            if abs(err_i) < tol:
                ...
                raise RuntimeError(
                    "Diverging in the {}'th iteration, err : {:.2e}".format(
                        i + 1, abs(err_i)
                    )
                )
                ...
                cx = np.reshape(
                    self.pss.cx, (self.Nx, -1), "F"
                )  # Reshape to 2-D format
    ...
    def toeplitz_setup(self, c, shp):
        ...
        toe_pos = [
            c[:, i].reshape((nrows, ncols)) for i in range(self.N, 2 * self.N + 1)
        ]
        ...
        toe = self.toe_l(*T)
        toe = np.reshape(
            np.transpose(toe, (0, 2, 1, 3)), (nrows * (2 * self.N + 1), -1)
        )
        return toe
    ...
    def calc_modal_props(self):
        ...
        if self.h:
            ...
            toeA = np.dot(
                np.dot(self.pss.Nblk, toeH)
                - np.dot(toeH, self.pss.Nblk)
                + np.dot(toeH, self.pss.toeA),
                toeH_inv,
            )
...
class ModalProps:
    ...
    def plot_eigs(self, ax):
        ...
        ax.plot(
            np.real(self.eig_fstrip),
            np.imag(self.eig_fstrip),
            "o",
            fillstyle="none",
            color="k",
            markersize=3,
            label=r"$\lambda_{fstrip}$",
        )
